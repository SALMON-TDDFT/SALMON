...
Subroutine Read_data
  ...
  if (comm_is_root(nproc_id_global)) then
    write(*,*) 'nproc_size_global=',nproc_size_global
...
!  aLx=ax*aL;    aLy=ay*aL;    aLz=az*aL
  ...
  NK_ave=NK/nproc_size_global; NK_remainder=NK-NK_ave*nproc_size_global
  NG_ave=NG/nproc_size_global; NG_remainder=NG-NG_ave*nproc_size_global

  if(is_symmetric_mode() == 1 .and. ENABLE_LOAD_BALANCER == 1) then
    call symmetric_load_balancing(NK,NK_ave,NK_s,NK_e,NK_remainder,nproc_id_global,nproc_size_global)
  else
    if (NK/nproc_size_global*nproc_size_global == NK) then
      NK_s=NK_ave*nproc_id_global+1
      NK_e=NK_ave*(nproc_id_global+1)
    else
      if (nproc_id_global < (nproc_size_global-1) - NK_remainder + 1) then
        NK_s=NK_ave*nproc_id_global+1
        NK_e=NK_ave*(nproc_id_global+1)
      else
        NK_s=NK-(NK_ave+1)*((nproc_size_global-1)-nproc_id_global)-NK_ave
        NK_e=NK-(NK_ave+1)*((nproc_size_global-1)-nproc_id_global)
      end if
    end if
    if(nproc_id_global == nproc_size_global-1 .and. NK_e /= NK) call err_finalize('prep. NK_e error')
  end if

  if (NG/nproc_size_global*nproc_size_global == NG) then
    NG_s=NG_ave*nproc_id_global+1
    NG_e=NG_ave*(nproc_id_global+1)
  else
    if (nproc_id_global < (nproc_size_global-1) - NG_remainder + 1) then
      NG_s=NG_ave*nproc_id_global+1
      NG_e=NG_ave*(nproc_id_global+1)
    else
      NG_s=NG-(NG_ave+1)*((nproc_size_global-1)-nproc_id_global)-NG_ave
      NG_e=NG-(NG_ave+1)*((nproc_size_global-1)-nproc_id_global)
    end if
  end if
  if(nproc_id_global == nproc_size_global-1 .and. NG_e /= NG) call err_finalize('prep. NG_e error')
