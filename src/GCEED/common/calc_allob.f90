...
if(ilsda==0.or.nproc_ob==1)then
  ...
  if(iparaway_ob==1)then
    if(nproc_id_spin<nproc_ob_spin(1))then
      iob_allob=nproc_id_kgrid*MST(1)/nproc_ob_spin(1)+iob
    else
      iob_allob=nproc_id_kgrid*MST(2)/nproc_ob_spin(2)+iob+MST(1)
    end if
  else if(iparaway_ob==2)then
    if(nproc_id_spin<nproc_ob_spin(1))then
      iob_allob=(iob-1)*nproc_ob_spin(1)+nproc_id_kgrid+1
    else
      iob_allob=(iob-1)*nproc_ob_spin(2)+nproc_id_kgrid+1+MST(1)
    end if
  end if
end if
